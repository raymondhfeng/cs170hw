...
\answer{\\
\textbf{Main Idea}\\
The main idea is similar to the main idea of binary representation. When translating a number to binary, after inserting the largest power of 2, the next power of two that can be inserted MUST be at most one less than the previously inserted power of two. We can modify this for this problem somewhat simply.
\textbf{Pseudocode}\\
exactChange(COST, DENOMS)\\
\tab sort DENOMS\\
\tab convert all elements in denoms to exponent with base 2.
\tab $i=\text{Largest index of denoms that fits in cost}$
}
